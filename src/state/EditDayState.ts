...
class EditDayState extends AppState {
    ...
    setupEditUI() {
        ...
        context.players.forEach((player, i) => {
            if (
                (i > 0 &&
                    player.firstname === context.players[i - 1].firstname) ||
                (i < context.players.length - 2 &&
                    player.firstname === context.players[i + 1].firstname)
            ) {
                player.tmpname =
                    player.firstname +
        ...
        if (this.options["mode"] === EditDayState.MODE_NEW) {
            ...
            $(".start-game").bind("click", e => {
                ...
                this.gameTimer.addEventListener(
                    "onTimerUpdate",
        ...
        $(".datepicker").pickadate({
            ...
            onSet: changed => {
                if (changed.select) {
                    let newdate = new Date(changed.select);
                    ...
                    this.controller.setGameDate(0, curdate.toString());
                }
            }
        });
        ...
        $(".collapsible").on(
            "click",
            ".accordion-header-blocker",
            this.stopAndPreventDefault
        );
            ...
            NewPlayerModal.show((player: NewPlayer) => {
                if (player) {
                    var template: HandlebarsTemplateDelegate = Handlebars.compile(
                        this.app.templates["partial_newday-item"]
                    );
                    ...
                    $("input.isPlaying, input.isLate").off(
                        "change",
                        this.onInputChange
                    );
                    $("button.point-input").off("click", this.onAddPoint);
                    $("button.point-subtract").off(
                        "click",
                        this.onSubtractPoint
                    );
                    ...
                    // add new listeners
                    $("input.isPlaying, input.isLate").on(
                        "change",
                        this.onInputChange
                    );
                    $("button.point-input").on("click", this.onAddPoint);
                    $("button.point-subtract").on(
                        "click",
                        this.onSubtractPoint
                    );
        ...
        $(".cancel-newday").bind("click", e => {
            ...
            if (confirm(msg)) {
                ...
                this.send(this.controller.getJSONString(), "update/all")
                    .done(data => {
        ...
        $(".save-newday").bind("click", e => {
            ...
            if (confirm(msg)) {
                ...
                for (var player in this.playersReceivedPowerup) {
                    ...
                    var multiple =
                        this.playersReceivedPowerup[player].length > 1;
                    ...
                    var p_message =
                        "<p>Hi {{firstname}},</p><p>You received the following powerup" +
                        (multiple ? "s" : "") +
                        " today: </p>" +
                        ...
                        " powerup" +
                        (multiple ? "s" : "") +
                        " will expire in 5 played games. Use " +
                        (multiple ? "them" : "it") +
                        " wisely!</p>";
                    ...
                    for (
                        var i = 0;
                        i < this.playersReceivedPowerup[player].length;
                        i++
                    ) {
                        var powerup_details = this.controller.getPowerupDetails(
                            this.playersReceivedPowerup[player][i]
                        );
                        var img = powerup_details.image
                            .split(".svg")
                            .join(".png");
                        ...
                        powerups.push(
                            "<tr>" +
                                ...
                                '"></td>' +
                                "<td><strong>" +
                                powerup_details.name +
                                ":</strong> " +
                                powerup_details.description +
                                "</td>" +
                                "</tr>"
                        );
                    }
    ...
    addBadge = e => {
        ...
        var template: HandlebarsTemplateDelegate = Handlebars.compile(
            this.app.templates["badges-selector"]
        );
        var context = {
            currentbadges: this.controller.getPlayerManualBadgesOnDay(
                playerid,
                0
            ),
        ...
        $(".new-badge").bind("click", e => {
            var badge: Badge = Badger.getBadgeByID(
            ...
            if (
                badge.id === "powerup" &&
                confirm("Generate powerup for player?")
            ) {
                var active = this.controller.getPlayerActivePowerupIDs(
                    playerid
                );
                var excludelist = [];
    ...
    usePowerup = e => {
        ...
        var playerid: string = $(e.currentTarget).attr("data-for");
        var template: HandlebarsTemplateDelegate = Handlebars.compile(
            this.app.templates["powerup-selector"]
        );
        var context = {
            ...
            players: this.controller
                ...
                .map(p => {
                    // rem
                    var powers = this.controller.getPlayerActivePowerupIDs(
                        p.id
                    );
        ...
        $(".do-usepower")
            .bind("click", e => {
                if (confirm("Use " + selectedpower + " powerup?")) {
                    this.controller.usePlayerPowerup(
                        playerid,
                        selectedpower,
                        useagainst
                    );
    ...
    endProgressTimer = (evt: object) => {
        this.gameTimer.removeEventListener(
            "onTimerUpdate",
            this.updateProgressTimer
        );
    ...
    onAddPoint = (e: any) => {
        ...
        if (!this.firstPointSet) {
            if (
                confirm(
                    "This is the first point, award player with Pointman badge?"
                )
            ) {
                this.firstPointSet = true;
        ...
        switch (inputtype) {
            ...
            case "checkbox":
                ...
                if (pointtype === "playing") {
                    var isPlaying = this.controller.getPlayerIsPlaying(
                        player,
                        0
                    );
                    ...
                    if (isPlaying !== checked) {
                        ...
                        currentTarget
                            .parents(".player-row")
                            [checked ? "addClass" : "removeClass"]("isPlaying");
                    }

                    $("[data-for='" + player + "']:not(.isPlaying)").prop(
                        "disabled",
                        !checked
                    );
                    ...
                    if (!checked) {
                        $(
                            "button.point-input[data-for='" +
                                player +
                                "']:not(.isPlaying)"
                        ).text("0");
                        $(
                            "[type='checkbox'][data-for='" +
                                player +
                                "']:not(.isPlaying)"
                        ).prop("checked", false);
                        $(".player-row a[data-for='" + player + "']").addClass(
                            "disabled"
                        );
    ...
    saveChanges = (create: boolean = false, callback?: Function) => {
        ...
        var cond: string[] = [];
        $(".conditions-select :selected").each((i, e) => {
            cond.push($(e).val());
        });
