...
class ScoreController {
    ...
    constructor(model: ScoreData) {
        ...
        if (
            ...
            this.model.scores[0].staging === true
        ) {
    ...
    getNewSeason = (options?: any): ScoreData => {
        var current_rankings = this.getPlayerRankings(0);
        var seasonIndex: number =
            this.model.season === 4 ? 1 : this.model.season + 1;
        ...
        var winner = this.model.players.find(
            p => p.id === current_rankings[0].id
        );
        ...
        var season: ScoreData = {
            ...
            last_winner: winner ? `${winner.firstname} ${winner.lastname}` : "",
            enableBadges: true,
    ...
    getAllActivePlayers = (): any[] => {
        ...
        for (var i = 0; i < this.model.players.length; ++i) {
            if (
                this.model.games[this.model.players[i].id] > 0 &&
    ...
    getGamePlayers = (day: number = 0): any[] => {
        ...
        for (var id in this.model.scores[day].values) {
            if (
                this.model.scores[day].values.hasOwnProperty(id) &&
                this.model.scores[day].values[id].played
            ) {
                var name = "";
                var score = 0;
                var rawscore = 0;
                ...
                for (var p in this.model.points) {
                    if (this.model.points.hasOwnProperty(p)) {
                        ...
                        if (player[p] > 0) {
                            rawscore +=
                                this.model.scores[day].values[id][p] *
                ...
                for (var b in this.model.bonuses) {
                    if (this.model.bonuses.hasOwnProperty(b) && b != "late") {
                        ...
                        if (player[b] > 0) {
                            bonuses +=
                                this.model.scores[day].values[id][b] *
                ...
                player.score =
                    this.model.scores[day].values[id].newtotal -
                    this.model.scores[day].values[id].lasttotal;
                ...
                if (this.model.scores[day].values[id].manualbadges) {
                    var receivedPowerups = this.getPlayerPowerupsReceivedOnDay(
                        id,
                        day
                    );
                    if (receivedPowerups.length > 0) {
                        ...
                        player.powerup =
                            this.model.scores[day].values[
                                id
                            ].manualbadges.indexOf("powerup") > -1;
    ...
    getDaysRemaining = (): number => {
        ...
        if (this.model.scores.length > 0) {
            if (
                new Date(this.model.scores[0].date).getDate() ===
                new Date().getDate()
            ) {
                includetoday = false;
            }
        ...
        return (
            DateUtil.getDaysRemaining("" + new Date(), this.model.end_date) -
            (includetoday ? 0 : 1)
        );
    };
    ...
    getPlayerScores = (day: number = 0): any[] => {
        ...
        for (var i = 0; i < this.model.players.length; ++i) {
            result.push({
                ...
                played: this.model.scores[day].values[this.model.players[i].id]
                    .played,
                late: this.model.scores[day].values[this.model.players[i].id]
                    .late
            });

            for (var p in this.model.points) {
                if (this.model.points.hasOwnProperty(p)) {
                    result[i][p] = this.model.scores[day].values[result[i].id][
                        p
                    ];
                }
            }
            ...
            for (var b in this.model.bonuses) {
                if (this.model.bonuses.hasOwnProperty(b) && b != "late") {
                    result[i][b] = this.model.scores[day].values[result[i].id][
                        b
                    ];
                }
            }
    ...
    getBadgeByID = (badgeid: string): Badge => {
        for (var i = 0; i < this.model.badges.length; ++i) {
            if (this.model.badges[i].id === badgeid)
                return this.model.badges[i];
        }
    ...
    getPlayerPowerupsReceivedOnDay = (
        playerid: string,
        day: number = 0
    ): any[] => {
    ...
    getPlayerScoreForDay = (playerid: string, day: number = 0): number => {
        ...
        if (
            ...
            this.model.scores[day].values[playerid].played
        ) {
            return (
                this.model.scores[day].values[playerid].newtotal -
                this.model.scores[day].values[playerid].lasttotal
            );
        }
